...
function Seats() {
  const params = useParams();
  const { id, date } = params;
  console.log(date, "dateeeeeeeeeeeeee");
  ...
  const generateSeatLayout = () => {
    ...
    const currentDateTime = new Date(); // Get the current date and time
    // const currentDateTime = new Date("2025-01-15T00:00:00.000Z"); 
    // Get the current date and time
    
    if (showDateTime <= currentDateTime) {
      return (
        <h4>
          Showtime has expired. Seats cannot be booked.Please Select Another
          Time
        </h4>
      );
    }
    ...
    return (
      <div>
        {showDateTime > currentDateTime ? (
                      ...
                      {row.cols.map((col, colIndex) => (
                        <div key={colIndex} className="seat-col">
                          {col.seats.map((seat, seatIndex) => {
                            ...
                            const seatNumber =
                              colIndex > 0 ? 11 + seatIndex : seatIndex + 1;
          ...
          <h5>
            {" "}
            time expired no seat available for this time slot select another
            time
          </h5>
        )}
      </div>
  ...
  return (
    <div className="selectseatpage">
          ...
          <div className="timecont">
            {screen?.movieSchedules.map((schedule, index) => (
              <h6
                key={index}
                className={
                  selectedTime?._id === schedule._id ? "selected" : "time"
                }
                onClick={() => {
                  setSelectedTime(schedule);
                  setSelectedSeat([]);
      ...
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        ...
        <div
          className="totalcont"
          style={{ borderLeft: "2px solid brown", marginRight: "100px" }}
        >
          <div className="total">
            <p>Movie Name : {movie?.movieName}</p>
            <p>Theatre : PixelFrames</p>
